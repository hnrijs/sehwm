...
static void update_monitors(void) {
    ...
    if (num_mons > 0) {
        current_ws = monitors[cur_mon].active_ws;
    }
}
...
static void tile(void) {
    for (int w = 0; w < NUM_WORKSPACES; w++) {
        ...
        if (has_fullscreen && fs_idx != -1) {
            ...
            XMoveResizeWindow(dpy, ws->clients[fs_idx].win, sx, sy, sw, sh);
...
static int add_client_to_ws(Window w, int target_ws) {
    ...
    if (float_by_default) {
        int sx = monitors[cur_mon].x;
        int sy = monitors[cur_mon].y;
        int sw = monitors[cur_mon].w;
        int sh = monitors[cur_mon].h;
        ws->clients[ws->count].x = sx + (sw - win_w) / 2;
        ws->clients[ws->count].y = sy + (sh - win_h) / 2;
    } else {
...
static void view_workspace(int target_ws) {
    ...
    for (int i = 0; i < num_mons; i++) {
        if (monitors[i].active_ws == target_ws) {
            mon_idx = i;
            break;
        }
    }
    ...
    if (mon_idx != -1) {
        ...
        XWarpPointer(dpy, None, root, 0, 0, 0, 0, 
                     monitors[cur_mon].x + monitors[cur_mon].w / 2, 
                     monitors[cur_mon].y + monitors[cur_mon].h / 2);
        XSync(dpy, False);
    } else {
        monitors[cur_mon].active_ws = target_ws;
        current_ws = target_ws;
    }
...
int main(void) {
    ...
    for (int i = 0; i < NUM_WORKSPACES; i++) ws_codes[i] = XKeysymToKeycode(dpy, ws_keysyms[i]);
    ...
    while (1) {
        ...
        switch (ev.type) {
            ...
            case EnterNotify: {
                ...
                for(int m = 0; m < num_mons; m++) {
                     if(ev.xcrossing.x_root >= monitors[m].x && ev.xcrossing.x_root < monitors[m].x + monitors[m].w &&
                        ev.xcrossing.y_root >= monitors[m].y && ev.xcrossing.y_root < monitors[m].y + monitors[m].h) {
                         target_mon = m;
                         break;
                     }
                }
            ...
            case ButtonPress: {
                ...
                for(int m = 0; m < num_mons; m++) {
                     if(ev.xbutton.x_root >= monitors[m].x && ev.xbutton.x_root < monitors[m].x + monitors[m].w &&
                        ev.xbutton.y_root >= monitors[m].y && ev.xbutton.y_root < monitors[m].y + monitors[m].h) {
                         cur_mon = m;
                         current_ws = monitors[cur_mon].active_ws;
                         break;
                     }
                    ...
                    if (is_dragging) {
                        ...
                        for(int m = 0; m < num_mons; m++) {
                            if(ev.xmotion.x_root >= monitors[m].x && ev.xmotion.x_root < monitors[m].x + monitors[m].w &&
                               ev.xmotion.y_root >= monitors[m].y && ev.xmotion.y_root < monitors[m].y + monitors[m].h) {
                                target_mon = m;
                                break;
                            }
                        }
