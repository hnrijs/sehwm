    ...
    while (1) {
        ...
        switch (ev.type) {
            ...
            case ButtonPress: {
                ...
                for (int i = 0; i < ws->count; i++) {
                    if (ws->clients[i].win == w && ws->clients[i].floating) {
                        target_client = &ws->clients[i];
                        break;
                    }
                }
                ...
                if (target_client) {
                    if (ev.xbutton.button == Button1 && (ev.xbutton.state & MODKEY)) {
                        ...
                        int orig_x = target_client->x;
                        int orig_y = target_client->y;
                        ...
                        while (1) {
                            ...
                            if (rel_ev.type == MotionNotify) {
                                ...
                                target_client->x = orig_x + dx;
                                target_client->y = orig_y + dy;
                                XMoveResizeWindow(dpy, target_client->win, target_client->x, target_client->y, target_client->width, target_client->height);
                        ...
                        int start_x = ev.xbutton.x_root;
                        int start_y = ev.xbutton.y_root;
                        int orig_w = target_client->width;
                        int orig_h = target_client->height;
                        ...
                        while (1) {
                            ...
                            if (rel_ev.type == MotionNotify) {
                                int dx = rel_ev.xmotion.x_root - start_x;
                                int dy = rel_ev.xmotion.y_root - start_y;
                                target_client->width = (orig_w + dx > 50) ? orig_w + dx : 50;
                                target_client->height = (orig_h + dy > 50) ? orig_h + dy : 50;
                                XMoveResizeWindow(dpy, target_client->win, target_client->x, target_client->y, target_client->width, target_client->height);
                            } else if (rel_ev.type == ButtonRelease) {
                                break;
                            }
