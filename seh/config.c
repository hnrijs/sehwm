...
static int is_transient_or_dialog(Window w) {
    ...
    Atom net_wm_type_splash = XInternAtom(dpy, "_NET_WM_WINDOW_TYPE_SPLASH", False);
...
static void tile(void) {
    ...
    for (int w = 0; w < NUM_WORKSPACES; w++) {
        ...
        for (int i = 0; i < workspaces[w].count; i++) {
            XMoveWindow(dpy, workspaces[w].clients[i].win, -3000, -3000);
        }
    }

    for (int i = 0; i < ws->count; i++) {
        if (ws->clients[i].fullscreen) {
            XMoveResizeWindow(dpy, ws->clients[i].win, 0, 0, sw, sh);
            XMapWindow(dpy, ws->clients[i].win);
    ...
    while (1) {
        ...
        switch (ev.type) {
            case ClientMessage: {
                ...
                Atom net_wm_fullscreen = XInternAtom(dpy, "_NET_WM_STATE_FULLSCREEN", False);

                if (ev.xclient.message_type == net_wm_state) {
                    Client *c = find_client(ev.xclient.window);
                    if (c) {
                        int action = ev.xclient.data.l[0]; 
                        int is_fullscreen_req = (ev.xclient.data.l[1] == net_wm_fullscreen || 
                                                 ev.xclient.data.l[2] == net_wm_fullscreen);

                        if (is_fullscreen_req) {
                            if (action == 1) c->fullscreen = 1;
                            else if (action == 0) c->fullscreen = 0;
                            else if (action == 2) c->fullscreen = !c->fullscreen;
            ...
            case ConfigureRequest: {
                XConfigureRequestEvent *ev_req = &ev.xconfigurerequest;
                XWindowChanges wc;
                wc.x = ev_req->x;
                wc.y = ev_req->y;
                wc.width = ev_req->width;
                wc.height = ev_req->height;
                wc.border_width = ev_req->border_width;
                wc.sibling = ev_req->above;
                wc.stack_mode = ev_req->detail;
                XConfigureWindow(dpy, ev_req->window, ev_req->value_mask, &wc);
                break;
            }
            case MapRequest: {
                ...
                if (c) {
                    ...
                    if (c->floating) {
                        if (ev.xbutton.button == Button1 && (ev.xbutton.state & MODKEY)) {
                            ...
                            while (1) {
                                XNextEvent(dpy, &rel_ev);
                                if (rel_ev.type == MotionNotify) {
                                    while (XCheckTypedEvent(dpy, MotionNotify, &rel_ev));

                                    int dx = rel_ev.xmotion.x_root - start_x;
                                    int dy = rel_ev.xmotion.y_root - start_y;
                                    c->x = orig_x + dx;
                                    c->y = orig_y + dy;
                                    XMoveResizeWindow(dpy, c->win, c->x, c->y, c->width, c->height);
                                    XSync(dpy, False);
                                } else if (rel_ev.type == ButtonRelease) {
                                    break;
                                }
                            }
                            ...
                            while (1) {
                                ...
                                if (rel_ev.type == MotionNotify) {
                                    ...
                                    int dx = rel_ev.xmotion.x_root - start_x;
                                    int dy = rel_ev.xmotion.y_root - start_y;
                                    c->width = (orig_w + dx > 50) ? orig_w + dx : 50;
                                    c->height = (orig_h + dy > 50) ? orig_h + dy : 50;
                                    XMoveResizeWindow(dpy, c->win, c->x, c->y, c->width, c->height);
                                    XSync(dpy, False);
                                } else if (rel_ev.type == ButtonRelease) {
                                    break;
